   <script type="text/javascript" src="inc/val.js" /></script>
    ...
    $("#splashtease").RSV({
        rules: [
        "required,emAdd,Valid Email.",
        "valid_email,emAdd,Valid Email.",
        ]
    });
});
	</script> 
   <script type="text/javascript">
    ...
    $pdo = new PDO('mysql:host=localhost;dbname=thedbnamehere', $u, $p);
    $pdo->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
    $sth = $pdo->prepare('SELECT dID, name FROM Department;');
    $sth->execute(array());
    /*----------------------------*/
    $pdo3 = new PDO('mysql:host=localhost;dbname=thedbnamehere', $u, $p);
    $pdo3->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
    $sth3 = $pdo3->prepare('SELECT dID, name FROM Department;');
    $sth3->execute(array());

    $pdo2 = new PDO('mysql:host=localhost;dbname=thedbnamehere;', $u, $p);
        ...
        <form action="inc/add_p_c_validate.php" method="post" id="addition"> 
     	 ...
     	 <div id="courseFields"><!--COURSE ///////// -->
		     ...
		     &nbsp;&nbsp;&nbsp;&nbsp;Department: <select name="deptName" id="deptName" style="width:350px;"><option disabled="disabled">Select..</option>
		       <?php while($row = $sth->fetch(PDO::FETCH_ASSOC)) {echo "<option value='".$row['dID']."'>".$row['name'].";";} ?></select>   
             ...
             <span class="sf" style="color:#29a4eb;">Professor Information</span><br />
	             ...
	             <div class="field required">
	              	&nbsp;&nbsp;&nbsp;&nbsp;Department: <select name="deptName" id="deptName" style="width:350px;"><option disabled="disabled">Select..</option>
	             	 <?php while($row3 = $sth3->fetch(PDO::FETCH_ASSOC)) {echo "<option value='".$row3['dID']."'>".$row3['name']."";} ?></select>
	              </div>
	               <input type="submit" name="submit" />       
             </div><!--/ProfFields-->
        </form> 
    ...
    <img align="left" style="float:left;" src="img/about_blurb.png" width="35" height="35" alt="About thedbnamehere" />
    ...
    <p class="sm"><br /><br /><a href="http://www.josephdickinson.com/joseph-dickinson-projects/">I established the idea</a>
     ...
     compared to one another. At the time (and still) the websites reviewing Drexel University were lacking a lot of functionality, or had hardly any reviews of professors here at Drexel. The ultimate goal, forever, for thedbnamehere is to help students make more informed choices when choosing courses at Drexel.</p>
    <br />
    <img style="float:left;" src="img/hope_to_address.png" width="35" height="35" alt="thedbnamehere Goals for Student Success" />
   ...
   <img class="left" src="img/contact.png" title="UReviewDU Legal Use of Website" width="30" height="30" /><h3 title="Email UReviewDU">Contact UReviewDU</h3>
    ...
    <div id="popup_name5" class="popup_block"><!--SUBSCRIBE (LOGIN) ///////// -->
	...
	<?php $usrBrowser = $_SERVER['HTTP_USER_AGENT']; $todayDt = date('Y-m-d');  ?>
	    ...
	    <input type="hidden" name="dt" value="<?php echo $todayDt; ?>" style="margin:0; padding:0;" /><br />
	    <input type="submit" name="submit" value="Submit" class="splashteasesub" id="submit" />
	</form>

 <!--
    <img align="left" class="left" src="img/userLogin.png" title="thedbnamehere Login" width="30" height="30" /> <h3 title="thedbnamehere User Login"> User Login</h3>
 ...
 -->
    ...
    <img class="left" src="img/legalSafe.png" width="34" height="34" alt="Safe Use of UReviewDU" /><h2>Use of site / Liability</h2>
	<p>We won't bore you with legal jargon, but in a nutshell, by posting on UReviewDU you will not hold us accountable for information you provide us.</p><br />
	<img class="left" src="img/privacy.png" width="34" height="34" alt="Safe Information UReviewDU" /><h2>Your Information</h2>
	<p>The information you give to us is in best hands and will not be redistributed EVER unless strictly communicated before hand.</p>
    </div><!--popup_name6-->