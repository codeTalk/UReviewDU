...
<?php
		$theCoMMID =  $row7['commID'];
			if ($sth7->rowCount()) {									 
			  		while($row7 = $sth7->fetch(PDO::FETCH_ASSOC)) {
			  			echo "<input type='hidden' value='<?php echo $theCoMMID; ?>' name='commID' /> ";
					}
				}
				else {echo "No User";}unset($sth7);
			?>		
			...
			<?php																		 
				...
				if ($sth2->rowCount()) {									 
  				while($row = $sth2->fetch(PDO::FETCH_ASSOC)) {				 
    				echo "{$row['date']} about <code><a href='course.php?cID={$row['cID']}'>{$row['prefix']} {$row['code']}</a>&nbsp;</code>  during  {$row['Qtr']},  {$row['Yr']} <span style='float:right; padding-right:5px;'><img src='img/report.png' /> <a class='report' href='report.php?commID={$row['CommID']}'>Report</a></span><br />{$row['info']} 
    				     ";
  					}														 
				}															 
				else {														 
