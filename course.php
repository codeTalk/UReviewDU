...
<head>
	...
	<meta name="description" content="Rate and compare courses and find the best professors at Drexel University. Anonymously rate professors without any guilt" />
	<meta name="keywords" content="Courses Drexel, Course Reviews Drexel, Rate Courses Drexel, Drexel University, PA, DU" />
			...
			<div id="course_name">
				...
				<?php
					...
					if($sth->rowCount() > 0) {
				    	...
				    	echo "<h2>{$row['prefix']} {$row['code']} Community Review</h2>";
						} else {
				    	echo "No results.";
					}
				unset($sth);
			...
			<div id="course_detail">
			 	...
			 	if ($sth2->rowCount()) {
  				while($row2 = $sth2->fetch(PDO::FETCH_ASSOC)) {
    				echo "<p>
    				<span style='float:right; padding-right:5px;'><img src='img/report.png' /> 
    				<a class='report' href='report.php?commID={$row2['CommID']}'>Report</a></span>
    				<img align='left' class='left' style='margin:5px;' src='img/courseComment.png'/> {$row2['info']} </p>
    				";
  					}
				}
				else {
  				echo "<h3 style='color:red;'> No comments found, please <a href='index.php'>add some</a> by searching for a professor.</h3></div>";
				}
				unset($sth2);
			...
			<div id="course_teachers_taught">
				...
				echo "<h3 class='orange'>Taught in the past by:</h3>"; 
			 	if ($sth4->rowCount()) {
  				...
  				echo "<h4 style='color:red;'>No Teachers have taught this course yet. Please <a href='#?w=500' rel='popup_name' class='poplight'>add some</a> .</h4>";
				}
				unset($sth4);
				?>
